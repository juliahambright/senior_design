...
def convert_wplist_meters(wp_list):
    wp_list_new = []

    for i in range(len(wp_list)):
        
        x = float(wp_list[i][1])/1000.0
        print("this is the new distance after /1000: %s /n"%x)
        list_to_append = [str(wp_list[0]), str(x), str(wp_list[2])]
        wp_list_new.append(list_to_append)
    print("this is the wp_list_new in conver to meters function: %s"%wp_list_new)

    return wp_list_new


def remove_zeros(wp_list):
    #counter for indexes that are removed
    l = 0
    #list of indexes with 0's to remove
    idx = []
    for i in range(len(wp_list)):
        if float(wp_list[i][1]) >0:
            idx.append(i)
...
def order_waypoints(wps, currentLat, currentLon, finalLoc):
...
def clean_wplist(wp_list, currentLat, currentLon, LocationGR):
    
        #wp_list = remove_zeros(wp_list)
        print("wptree list before edits: %s"%wp_list)

        #wp_list = convert_wplist_meters(wp_list)

        #orders the waypoints
        wp_list = order_waypoints(wp_list,currentLat, currentLon, LocationGR)
...
#this time function will let us turn off the bot if it has been running too long without arriving (safety feature)
x = time.time()

#starts the lidar so that it can close the process and restart it everytime in the while loop
#proc, stdoutdata = start_lidar()

#main while loop to run bot
while distance_to_current_waypoint(final_loc)>5:
    ...
    print("Distance to current waypoing: %s\n"%distance_to_current_waypoint(next_loc))

    if distance_to_current_waypoint(next_loc)<1000:
        ...
        #Uses function to start the lidar 
#-----------------------------------------------------------------------
        #stop the lidar process first before reopening it
        #proc.stdout.close()
        #proc, stdoutdata = start_lidar()
#------------------------------------------------------------------------
        var = os.popen('sudo ./ultra_simple').read()
        print("this is var: %s"%var)
        time.sleep(5)
        match = re.findall("theta: (?P<theta>[\d.]+(?= )) , distance: (?P<distance>[\d.]+(?= )) , Waypoint (?P<waypoint>[\d.]+(?= ))", var.decode('utf-8'))
        
        #if waypoint is found
        if len(match)>0:
            #sets list equal to the waypoints found by the lidar in this format [theta, dist, waypoint num]
            wptree_loc_list = match
            
            #gets the current lattitude and longitude
            currentLat = vehicle.location.global_frame.lat
            currentLon = vehicle.location.global_frame.lon
            
            #cleans the waypoint list: removes zeros, converts dist to meters, and orders waypoints
            orderedWP_list = order_waypoints(wptree_loc_list, currentLat, currentLon, final_loc)
            
            wptree["loc"+str(j)] = orderedWP_list
            #picks the closests waypoint to the final dest
            bestWP = orderedWP_list[0]
            
            #converts the distance and theta into a lattitude and longitude point relative to the current location
            dist = bestWP[1]
            theta = bestWP[0]
            print("this is theta from bestWP[0]: %s /n"%theta)
